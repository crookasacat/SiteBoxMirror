...
// end /CodeMirror2/mode/htmlmixed/htmlmixed.js


// /CodeMirror2/mode/xmlpure/xmlpure.js
CodeMirror.defineMode("xmlpure",function(b,s){var A="error";var B="comment";var x="comment";var y="tag";var w="attribute";var D="string";var C="atom";var z="string";var G="!instruction";var E="!cdata";var F="!comment";var H="!text";var c={"!cdata":true,"!comment":true,"!text":true,"!instruction":true};var e=b.indentUnit;function a(K,J,I){J.tokenize=I;return I(K,J)}function d(J,K,I){return function(M,L){while(!M.eol()){if(M.match(K)){u(L);L.tokenize=I;break}M.next()}return J}}function v(K,L){var J=c.hasOwnProperty(L)||(K.context&&K.context.doIndent);var I={tagName:L,prev:K.context,indent:K.context?K.context.indent+e:0,lineNumber:K.lineNumber,indented:K.indented,noIndent:J};K.context=I}function u(J){if(J.context){var I=J.context;J.context=I.prev;return I}return null}function f(I){return I.sol()||I.string.charAt(I.start-1)==" "||I.string.charAt(I.start-1)=="\t"}function m(J,I){if(J.eat("<")){if(J.eat("?")){v(I,G);I.tokenize=r;return B}else{if(J.match("!--")){v(I,F);return a(J,I,d(x,"-->",m))}else{if(J.eatSpace()||J.eol()){J.skipToEnd();return A}else{I.tokenize=p;return y}}}}J.skipToEnd();return A}function p(M,L){var K=M.pos;if(M.match(/^[a-zA-Z_:][-a-zA-Z0-9_:.]*/)){var N=M.string.substring(K,M.pos);v(L,N);L.tokenize=n;return y}else{if(M.match(/^\/[a-zA-Z_:][-a-zA-Z0-9_:.]*( )*>/)){var I=M.string.substring(K+1,M.pos-1).trim();var J=u(L);L.tokenize=L.context==null?m:o;if(J==null||I!=J.tagName){return A}return y}else{L.tokenize=L.context==null?m:o;M.eatWhile(/[^>]/);M.eat(">");return A}}M.skipToEnd();return null}function n(J,I){if(J.match(/^\/>/)){u(I);I.tokenize=I.context==null?m:o;return y}else{if(J.eat(/^>/)){I.tokenize=o;return y}else{if(f(J)&&J.match(/^[a-zA-Z_:][-a-zA-Z0-9_:.]*( )*=/)){I.tokenize=g;return w}}}I.tokenize=I.context==null?m:m;J.eatWhile(/[^>]/);J.eat(">");return A}function g(K,J){var I=K.next();if(I!='"'&&I!="'"){K.skipToEnd();J.tokenize=n;return A}J.tokParams.quote=I;J.tokenize=h;return D}function h(K,J){var I="";while(!K.eol()){I=K.next();if(I==J.tokParams.quote){J.tokenize=n;return D}else{if(I=="<"){K.skipToEnd();J.tokenize=n;return A}else{if(I=="&"){I=K.next();if(I==";"){K.skipToEnd();J.tokenize=n;return A}while(!K.eol()&&I!=";"){if(I=="<"){K.skipToEnd();J.tokenize=n;return A}I=K.next()}if(K.eol()&&I!=";"){K.skipToEnd();J.tokenize=n;return A}}}}}return D}function o(J,I){if(J.eat("<")){if(J.match("?")){v(I,G);I.tokenize=r;return B}else{if(J.match("!--")){v(I,F);return a(J,I,d(x,"-->",I.context==null?m:o))}else{if(J.match("![CDATA[")){v(I,E);return a(J,I,d(C,"]]>",I.context==null?m:o))}else{if(J.eatSpace()||J.eol()){J.skipToEnd();return A}else{I.tokenize=p;return y}}}}}else{if(J.eat("&")){J.eatWhile(/[^;]/);J.eat(";");return z}else{v(I,H);I.tokenize=t;return null}}I.tokenize=I.context==null?m:o;J.skipToEnd();return null}function t(J,I){J.eatWhile(/[^<]/);if(!J.eol()){u(I);I.tokenize=o}return C}function r(J,I){if(J.match("xml",true,true)){if(I.lineNumber>1||J.pos>5){I.tokenize=m;J.skipToEnd();return A}else{I.tokenize=l;return B}}if(f(J)||J.match("?>")){I.tokenize=m;J.skipToEnd();return A}I.tokenize=q;return B}function q(J,I){J.eatWhile(/[^?]/);if(J.eat("?")){if(J.eat(">")){u(I);I.tokenize=I.context==null?m:o}}return B}function l(J,I){I.tokenize=i;if(f(J)&&J.match(/^version( )*=( )*"([a-zA-Z0-9_.:]|\-)+"/)){return B}J.skipToEnd();return A}function i(J,I){I.tokenize=k;if(f(J)&&J.match(/^encoding( )*=( )*"[A-Za-z]([A-Za-z0-9._]|\-)*"/)){return B}return null}function k(J,I){I.tokenize=j;if(f(J)&&J.match(/^standalone( )*=( )*"(yes|no)"/)){return B}return null}function j(J,I){I.tokenize=m;if(J.match("?>")&&J.eol()){u(I);return B}J.skipToEnd();return A}return{electricChars:"/[",startState:function(){return{tokenize:m,tokParams:{},lineNumber:0,lineError:false,context:null,indented:0}},token:function(J,I){if(J.sol()){I.lineNumber++;I.lineError=false;I.indented=J.indentation()}if(J.eatSpace()){return null}var K=I.tokenize(J,I);I.lineError=(I.lineError||K=="error");return K},blankLine:function(I){I.lineNumber++;I.lineError=false},indent:function(I,J){if(I.context){if(I.context.noIndent==true){return}if(J.match(/^<\/.*/)){return I.context.indent}if(J.match(/^<!\[CDATA\[/)){return 0}return I.context.indent+e}return 0},compareStates:function(I,J){if(I.indented!=J.indented){return false}for(var K=I.context,L=J.context;;K=K.prev,L=L.prev){if(!K||!L){return K==L}if(K.tagName!=L.tagName){return false}}}}});CodeMirror.defineMIME("application/xml","purexml");CodeMirror.defineMIME("text/xml","purexml");
// end /CodeMirror2/mode/xmlpure/xmlpure.js


// /CodeMirror2/SiteBoxMirror/mysql/mysql.js
CodeMirror.defineMode("mysql",function(b,h){var d=b.indentUnit,f=h.keywords,c=h.functions,o=h.types,g=h.multiLineStrings;var e=/[+\-*&%=<>!?:\/|]/;function a(r,q,p){q.tokenize=p;return p(r,q)}var n;function i(q,p){n=q;return p}function j(r,q){var p=r.next();if(p=="`"||p=='"'){return a(r,q,l(p))}else{if(p=='"'||p=="'"){return a(r,q,m(p))}else{if(/[\(\),;\.]/.test(p)){return i(p)}else{if(/\d/.test(p)){r.eatWhile(/[\w\.]/);return i("number","number")}else{if(p=="/"){if(r.eat("*")){return a(r,q,k)}else{r.eatWhile(e);return i("operator","operator")}}else{if(p=="#"){r.skipToEnd();return i("comment","comment")}else{if(p=="@"){r.eatWhile(/[\w\d\$_]/);return i("word","variable")}else{if(e.test(p)){r.eatWhile(e);return i("operator","operator")}else{r.eatWhile(/[\w\$_]/);if(f&&f.propertyIsEnumerable(r.current().toLowerCase())){return i("keyword","keyword")}if(c&&c.propertyIsEnumerable(r.current().toLowerCase())){return i("keyword","builtin")}if(o&&o.propertyIsEnumerable(r.current().toLowerCase())){return i("keyword","variable-2")}return i("word","word")}}}}}}}}}function m(p){return function(u,t){var r=false,s,q=false;while((s=u.next())!=null){if(s==p&&!r){q=true;break}r=!r&&s=="\\"}if(q||!(r||g)){t.tokenize=j}return i("string","string")}}function l(p){return function(r,q){while((next=r.next())!=null){if(next==p){end=true;break}}if(end){q.tokenize=j}return i("identifier","attribute")}}function k(s,r){var q=false,p;while(p=s.next()){if(p=="/"&&q){r.tokenize=j;break}q=(p=="*")}return i("comment","comment")}return{startState:function(p){return{tokenize:j,startOfLine:true}},token:function(q,p){if(q.eatSpace()){return null}var r=p.tokenize(q,p);return r}}});(function(){function a(g){var f={},h=g.split(" ");for(var e=0;e<h.length;++e){f[h[e]]=true}return f}var c="alter auto_increment grant revoke primary key table start top transaction select update insert delete create describe from into values where join inner left natural and or in not xor like using on order group by asc desc limit offset union all as distinct set commit rollback replace view database separator if exists null truncate status show lock unique having drop procedure begin end delimiter call else leave declare temporary then engine";var b="abs acos adddate aes_encrypt aes_decrypt ascii asin atan atan2 avg benchmark bin bit_and bit_count bit_length bit_or cast ceil ceiling char_length character_length coalesce concat concat_ws connection_id conv convert cos cot count curdate current_date current_time current_timestamp current_user curtime database date_add date_format date_sub dayname dayofmonth dayofweek dayofyear decode degreesdes_encrypt des_decrypt elt encode encrypt exp export_set extract field find_in_set floor format found_rows from_days from_unixtime get_lock greatest group_unique_users hex ifnull inet_aton inet_ntoa instr interval is_free_lock isnull last_insert_id lcase least left length ln load_file locate log log2 log10 lower lpad ltrim make_set master_pos_wait max md5 mid min mod monthname now nullif oct octet_length ord password period_add period_diff pi position pow power quarter quote radians rand release_lock repeat reverse right round rpad rtrim sec_to_time session_user sha sha1 sign sin soundex space sqrt std stddev strcmp subdate substring substring_index sum sysdate system_user tan time_format time_to_sec to_days trim ucase unique_users unix_timestamp upper user version week weekday yearweek";var d="bigint binary bit blob bool char character date datetime dec decimal double enum float float4 float8 int int1 int2 int3 int4 int8 integer long longblob longtext mediumblob mediumint mediumtext middleint ncharnumeric real set smallint text time timestamp tinyblob tinyint tinytext varbinary varchar year";CodeMirror.defineMIME("text/x-mysql",{name:"mysql",keywords:a(c),functions:a(b),types:a(d)})}());
// end /CodeMirror2/SiteBoxMirror/mysql/mysql.js
